...
# ==============================================================================
# MÓDULOS DE LANÇAMENTOS (NOTAS, MANUAL, FOLHA E OFX)
# ==============================================================================
...
def renderizar_modulo_lancamentos():
    ...
    with abas_operacionais[0]:
        ...
        if df_plano.empty:
            ...
            with str.form("form_manual", clear_on_submit=True):
                ...
                c_debito = col4.selectbox("Conta de Débito (Aplicação)", options=opcoes_contas, format_func=lambda x: f"{x} - {df_plano[df_plano['codigo']==x]['descricao'].values[0] if len(df_plano[df_plano['codigo']==x]['descricao'].values) > 0 else ''}")
                c_credito = col5.selectbox("Conta de Crédito (Origem)", options=opcoes_contas, format_func=lambda x: f"{x} - {df_plano[df_plano['codigo']==x]['descricao'].values[0] if len(df_plano[df_plano['codigo']==x]['descricao'].values) > 0 else ''}")
    ...
    with abas_operacionais[1]:
        ...
        with str.form("form_nota", clear_on_submit=True):
            ...
            lista_part = df_part['id'].tolist() if not df_part.empty else []
            partic = col2.selectbox("Participante / Fornecedor", options=lista_part, format_func=lambda x: df_part[df_part['id']==x]['nome'].values[0] if x in df_part['id'].values else "Nenhum participante")
            
            lista_acum = df_acum['id'].tolist() if not df_acum.empty else []
            acumula = col3.selectbox("Acumulador / Operação", options=lista_acum, format_func=lambda x: f"Operação {x}" if x != 0 else "Nenhum acumulador")
            
            col4, col5 = str.columns(2)
            v_bruto = col4.number_input("Valor Bruto da Nota (R$)", min_value=0.00, step=50.0)
            c_contrapartida = col5.selectbox("Conta de Contrapartida da Despesa/Estoque", options=df_plano['codigo'].tolist() if not df_plano.empty else [""])
...
# ==============================================================================
# DEMONSTRAÇÕES CONTÁBEIS E MENU PRINCIPAL
# ==============================================================================
...
def renderizar_demonstracoes():
    ...
    with sub_abas[1]:
        ...
        receita_bruta = obter_saldo_por_prefixo("3.1")
...
def main():
    ...
    opcao_menu = str.sidebar.radio("Navegação do Sistema", ["Escrituração Contábil", "Demonstrações Oficiais", "Informações da Aplicação"])
    str.sidebar.markdown("---")
    ...
    if opcao_menu == "Escrituração Contábil":
        ...
        str.info("Sistema ativo e conectado de forma estável à base remota do Supabase.")
