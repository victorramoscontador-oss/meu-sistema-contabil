...
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImhjZ3R5dXd6emh6aGV0dmppaml4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODU0MTc3NDAsImV4cCI6MjEwMDk5Mzc0MH0._b3waLLjoYLL_VyCWGaksovJKr4ZZifo2EA2z9vRpA"
...
def renderizar_modulo_lancamentos(empresa_id): 
    ...
    def formatar_opcao_conta(codigo): 
        if not codigo or df_plano.empty: return "" 
        linha = df_plano[df_plano['codigo'] == codigo] 
        return f"{codigo} - {linha['descricao'].values[0]}" if not linha.empty else codigo 
    ...
    with aba1: 
        ...
        with str.form("form_manual", clear_on_submit=True): 
            ...
            if not historico_lan: historico_lan = str.text_input("Histórico Manual") 
    ...
    with aba3: 
        ...
        if arquivo_ofx is not None: 
            extrato_dados = [{"Data": "2026-07-10", "Documento": "TRF 4930", "Valor": 150.00}, {"Data": "2026-07-12", "Documento": "PIX RECEB VENDA", "Valor": 1200.00}] 
            analise_regras = [] 
...
def renderizar_demonstracoes(empresa_id, nome_empresa): 
    ...
    str.markdown('<button onclick="window.print()" style="background-color:#00ff66;color:#0b2216;padding:10px 20px;border:none;border-radius:5px;font-weight:bold;cursor:pointer;margin-bottom:15px;">Imprimir / Salvar em PDF</button>', unsafe_allow_html=True) 
    aba_rep1, aba_rep2, aba_rep3 = str.tabs(["Balancete por Níveis", "DRE Oficial Dedutiva", "Balanço Patrimonial Vertical"]) 
    
    with aba_rep1: 
        str.markdown(f'<div class="print-area"><h2>BALANCETE DE VERIFICAÇÃO</h2><p><b>Empresa:</b> {nome_empresa}</p><p><b>Período:</b> {d_ini.strftime("%d/%m/%Y")} - {d_fim.strftime("%d/%m/%Y")}</p><hr/></div>', unsafe_allow_html=True) 
    ...
    with aba_rep2: 
        str.subheader("Demonstração do Resultado do Exercício Estruturada") 
        if not df_balancete.empty: 
            ...
            """) 
        else: str.info("Sem movimentações de contas de resultado registradas no diário.") 
    ...
    with aba_rep3: 
        ...
        else: str.info("Aguardando consolidação de lançamentos patrimoniais.") 

def renderizar_modulo_cadastros(empresa_id): 
    str.header("Painel de Cadastros Estruturais") 
    aba_emp, aba_contas, aba_part, aba_acum, aba_hist, aba_ofx = str.tabs(["Empresas", "Contas Contábeis", "Clientes/Fornecedores", "Acumuladores Fiscais", "Históricos Padrão", "Regras OFX"]) 
    df_plano = buscar_plano_contas(empresa_id) 
